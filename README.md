...
また、 ある $c_F$ があって $[c_F, \infty) \subset L$ となることをいうには、$L$ が閉集合であることも示す必要があるが、その点はまだ形式化していない（現状でも $c_F$ があって $(c_F, \infty) \subset L$ となることはいえる）

## 問題選定について
上記の $c_F$ は Freiman(1975) によって決定されたため、Freiman の定数と呼ばれている。ところが、その証明は旧ソ連の地方大学の出版物として発行されたため入手性が極めて悪い上に、100ページ以上の長大な計算を伴うため、事実上半世紀以上誰にも検証されていない不健全な情況が続いている。
...
Mathlib の固定バージョンと Lean toolchain は `lake-manifest.json` および
`lean-toolchain` に記録している。リポジトリのルートで次を実行すると、
全形式化を型検査できる。
